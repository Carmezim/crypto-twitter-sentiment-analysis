...
def get_glove_vectors(vocab):
    ...
    glove_vectors = {}
    found = 0
    with io.open(GLOVE_FILE, 'r', encoding='utf-8') as glove_file:
...
def process_tweets(csv_file, test_file=False):
    ...
    with io.open(csv_file, 'r', encoding="utf-8") as csv:
        lines = csv.readlines()
        total = len(lines)
...
if __name__ == '__main__':
    ...
    max_length = 40
    filters = 600
    kernel_size = 4 # incremented by 1 from default
    ...
    if train:
        ...
        print(model.summary())
        model.fit(tweets, labels, batch_size=256, epochs=5,
                  validation_split=0.1, shuffle=True, callbacks=[checkpoint, reduce_lr])
    else:
